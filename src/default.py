...
niter   = 100
# Number of chains or walkers to be used to explore parameter space
nchains = 100
...
# For multiplanet fits, the scaled semi-major axis of all fitted planets
...
# Flags to control parameter priors
# For multiplanet fits, this variable has to have N elements as planets we are fitting
fit_t0  = ['f']
fit_P   = ['f']
fit_e   = ['f']
fit_w   = ['f']
...
# on the input stellar parameters
# BE AWARE THAT YOU HAVE TO BE CONFIDENT ABOUT YOUR STELLAR PARAMETERS TO USE IT!
# For multi-planet fits, a_from_kepler has to be N elements as planets are in the fit
a_from_kepler = [False]
...
# If the input file only has two columns, time and flux, we can specify the
...
# For multiplanet fits, this variable has to have N elemens as planets we are fitting
span_tr = [0.0]
...
# If True, the code creates authomatic priors for the RV offsets
